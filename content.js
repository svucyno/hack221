...
async function extractLinkedInProfileAsync() {
  ...
  const filterSkillStrict = (skill) => {
    ...
    if (/^\d+$/.test(skill.trim())) return false;
    if (!/[a-zA-Z]/.test(skill)) return false;

    if (skill.length < 6 && skill === skill.toUpperCase() && /[A-Z]/.test(skill)) return false;

    const lower = skill.toLowerCase();
    const exactBanned = ['live', 'top', 'new', 'hot', 'pro', 'ads', 'app'];
    if (exactBanned.includes(lower.trim())) return false;
